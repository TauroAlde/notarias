module CustomValidators
  class PermissionValidator < ActiveModel::Validator
    def validate(record)
      if record.featurette.blank? && record.featurette_object.blank?
      ...
      elsif record.featurette && record.featurette_object.present?
    ...
    def find_permission(record)
      ...
      query = query.where("action = ? OR action = ?", Authorizer::MANAGE, record.action)
      query
    end
  end
end